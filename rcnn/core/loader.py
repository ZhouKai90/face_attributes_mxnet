...
class CropLoader(mx.io.DataIter):
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            del data_shape['im_info']
            feat_shape_list = []
            #针对原始输入图像，针对不同stride的下采样，推断出不同的shape
            #例如输入图像(1,2,640,640),stride=32,即下采样了32倍，所以得到的feat_shape=(1,3,20,20)
            for s in range(len(self.feat_stride)):
                _, feat_shape, _ = self.feat_sym[s].infer_shape(**data_shape)
                feat_shape = [int(i) for i in feat_shape[0]]
                feat_shape_list.append(feat_shape)
            ...
            for k in self.label_name:
              label[k] = label_dict[k]      #将得到的生成的rpn相关的label加入到之前的label中
        #多batch下的data和label的准备
        all_data = dict()
        for key in self.data_name:
            all_data[key] = tensor_vstack([batch[key] for batch in data_list])
