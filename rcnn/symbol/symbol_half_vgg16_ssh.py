...
def get_feat_down(conv_feat):
	...
	P5_up   = mx.symbol.UpSampling(P5, scale=2, sample_type='nearest', workspace=256, name='P5_upsampling', num_args=1)
	...
	P4_up   = mx.symbol.UpSampling(P4, scale=2, sample_type='nearest', workspace=256, name='P4_upsampling', num_args=1)
...
def get_ssh_conv(data):
	...
	conv1_1 = mx.symbol.Convolution(
		data=data, kernel=(3, 3), pad=(1, 1), num_filter=32, workspace=1024, name="conv1_1")
	relu1_1 = mx.symbol.Activation(data=conv1_1, act_type="relu", name="relu1_1")
	conv1_2 = mx.symbol.Convolution(
		data=relu1_1, kernel=(3, 3), pad=(1, 1), num_filter=32, workspace=1024, name="conv1_2")
	relu1_2 = mx.symbol.Activation(data=conv1_2, act_type="relu", name="relu1_2")
	pool1 = mx.symbol.Pooling(
		data=relu1_2, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool1")
	# group 2
	conv2_1 = mx.symbol.Convolution(
		data=pool1, kernel=(3, 3), pad=(1, 1), num_filter=64, workspace=1024, name="conv2_1")
	relu2_1 = mx.symbol.Activation(data=conv2_1, act_type="relu", name="relu2_1")
	conv2_2 = mx.symbol.Convolution(
		data=relu2_1, kernel=(3, 3), pad=(1, 1), num_filter=64, workspace=1024, name="conv2_2")
	relu2_2 = mx.symbol.Activation(data=conv2_2, act_type="relu", name="relu2_2")
	pool2 = mx.symbol.Pooling(
		data=relu2_2, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool2")
	# group 3
	conv3_1 = mx.symbol.Convolution(
		data=pool2, kernel=(3, 3), pad=(1, 1), num_filter=128, workspace=1024, name="conv3_1")
	relu3_1 = mx.symbol.Activation(data=conv3_1, act_type="relu", name="relu3_1")
	conv3_2 = mx.symbol.Convolution(
		data=relu3_1, kernel=(3, 3), pad=(1, 1), num_filter=128, workspace=1024, name="conv3_2")
	relu3_2 = mx.symbol.Activation(data=conv3_2, act_type="relu", name="relu3_2")
	conv3_3 = mx.symbol.Convolution(
		data=relu3_2, kernel=(3, 3), pad=(1, 1), num_filter=128, workspace=1024, name="conv3_3")
	relu3_3 = mx.symbol.Activation(data=conv3_3, act_type="relu", name="relu3_3")
	pool3 = mx.symbol.Pooling(
		data=relu3_3, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool3")
	# group 4
	conv4_1 = mx.symbol.Convolution(
		data=pool3, kernel=(3, 3), pad=(1, 1), num_filter=256, workspace=1024, name="conv4_1")
	relu4_1 = mx.symbol.Activation(data=conv4_1, act_type="relu", name="relu4_1")
	conv4_2 = mx.symbol.Convolution(
		data=relu4_1, kernel=(3, 3), pad=(1, 1), num_filter=256, workspace=1024, name="conv4_2")
	relu4_2 = mx.symbol.Activation(data=conv4_2, act_type="relu", name="relu4_2")
	conv4_3 = mx.symbol.Convolution(
		data=relu4_2, kernel=(3, 3), pad=(1, 1), num_filter=256, workspace=1024, name="conv4_3")
	relu4_3 = mx.symbol.Activation(data=conv4_3, act_type="relu", name="relu4_3")
	pool4 = mx.symbol.Pooling(
		data=relu4_3, pool_type="max", kernel=(2, 2), stride=(2, 2), name="pool4")
	# group 5
	conv5_1 = mx.symbol.Convolution(
		data=pool4, kernel=(3, 3), pad=(1, 1), num_filter=256, workspace=1024, name="conv5_1")
	relu5_1 = mx.symbol.Activation(data=conv5_1, act_type="relu", name="relu5_1")
	conv5_2 = mx.symbol.Convolution(
		data=relu5_1, kernel=(3, 3), pad=(1, 1), num_filter=256, workspace=1024, name="conv5_2")
	relu5_2 = mx.symbol.Activation(data=conv5_2, act_type="relu", name="relu5_2")
	conv5_3 = mx.symbol.Convolution(
		data=relu5_2, kernel=(3, 3), pad=(1, 1), num_filter=256, workspace=1024, name="conv5_3")
	...
	F1 = 128		#Detection Module M2 and M3 out put channels
	F2 = 64		#Detection Module M1 out put channels
	_bwm = 1.0
	...
	conv5_128 = conv_act_layer(relu5_3, 'ssh_m2_red_conv',
							   F2, kernel=(1, 1), pad=(0, 0), stride=(1, 1), act_type='relu', bias_wd_mult=_bwm)
	#上采样之后和conv4_128进行特征融合,增加feature map特征的丰富性．借鉴了FPN的思想
	...
	for stride in config.RPN_FEAT_STRIDE:
		...
		if config.TRAIN.RPN_ENABLE_OHEM < 0:
			...
			if config.TRAIN.RPN_ENABLE_OHEM == 2:
				label, kpoint_weight, bbox_weight = mx.sym.Custom(op_type='rpn_fpn_ohem', stride=int(stride), cls_score=rpn_cls_score_reshape,
																  bbox_weight=bbox_weight, kpoint_weight=kpoint_weight, labels=label)
			...
			ret_group.append(mx.sym.BlockGrad(label, name='stride%d_label'%stride))
			ret_group.append(rpn_bbox_loss)
			ret_group.append(mx.sym.BlockGrad(bbox_weight, name='stride%d_bbox_weight'%stride))
			ret_group.append(rpn_kpoint_loss)
			ret_group.append(mx.sym.BlockGrad(kpoint_weight, name='stride%d_kpoint_weight'%stride))
